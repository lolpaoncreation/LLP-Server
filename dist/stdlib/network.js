...
exports.runtimeValToJs = runtimeValToJs;
...
function jsToRuntimeVal(val) {
    ...
    if (Array.isArray(val)) {
        ...
        listVal.Remove = (idx) => { if (idx >= 0 && idx < listVal.elements.length)
            listVal.elements.splice(idx, 1); };
...
function registerNetwork(env) {
    const deviceMgr = device_1.DeviceIdentityManager.getInstance();
    const sessionMgr = session_1.SessionManager.getInstance();
    // Global RPC, Push event, and Asymmetric Device Security stores
    const rpcHandlers = new Map();
    const devicePushQueues = new Map();
    ...
    const queueEventForDevice = (devId, eventData) => {
    ...
    // --------------------------------------------------------------------------
    const clientObj = new instance_1.Instance("HttpClientService");
    clientObj.Name = "Client";
    let serverBaseUrl = "";
    let remoteSessionId = "";
    // Helper to locate http_sync_worker.js
    const getSyncWorkerPath = () => {
        const candidates = [
    ...
    const executeSyncRequest = (method, targetUrl, body, customHeaders) => {
    ...
    clientObj.SetProperty("GetServerUrl", {
        type: "native_fn",
        call: () => (0, values_1.MK_STRING)(serverBaseUrl)
    });
    ...
    clientObj.SetProperty("IsConnected", {
        ...
        call: () => {
            ...
            return (0, values_1.MK_BOOL)(res.ok || res.status === 200 || res.status === 404);
        }
    });
    ...
    // --------------------------------------------------------------------------
    const serverObj = new instance_1.Instance("HttpServerService");
    serverObj.Name = "Server";
    let httpServer = null;
    let activePort = 0;
    let requireDeviceSecurity = true;
    let authSecret = (0, crypto_1.getProjectEncryptionKey)();
    ...
    serverObj.SetProperty("RegisterRPC", {
        ...
        call: (args) => {
            let service = "Default";
            let method = "";
            let handler = (0, values_1.MK_NULL)();
            if (args.length >= 3) {
    ...
    const createRequestInstance = (req, bodyData) => {
        const reqInst = new instance_1.Instance("HttpRequest");
        ...
        reqInst.SetProperty("Method", (0, values_1.MK_STRING)(method));
        reqInst.SetProperty("Path", (0, values_1.MK_STRING)(url));
        reqInst.SetProperty("Body", (0, values_1.MK_STRING)(bodyData));
        reqInst.SetProperty("DeviceId", (0, values_1.MK_STRING)(deviceId));
        reqInst.SetProperty("DeviceToken", (0, values_1.MK_STRING)(deviceToken));
        reqInst.SetProperty("SessionId", (0, values_1.MK_STRING)(sessionId));
        reqInst.SetProperty("ClientIp", (0, values_1.MK_STRING)(clientIp));
        reqInst.SetProperty("Headers", (0, values_1.MK_STRING)(JSON.stringify(req.headers)));
        return reqInst;
    };
    ...
    serverObj.SetProperty("RequireDevice", {
        ...
        call: (args) => {
            ...
            return (0, values_1.MK_BOOL)(requireDeviceSecurity);
        }
    });
    ...
    serverObj.SetProperty("GetConnectedClients", {
        ...
        call: () => {
            ...
            return (0, values_1.MK_STRING)(JSON.stringify(list));
        }
    });
    ...
    serverObj.SetProperty("KickClient", {
        ...
        call: (args) => {
            ...
            return (0, values_1.MK_BOOL)(removed);
        }
    });
    ...
    serverObj.SetProperty("IsDeviceRevoked", {
        ...
        call: (args) => {
            const devId = args[0]?.type === "string" ? args[0].value : "";
            if (bannedDevices.has(devId))
                return (0, values_1.MK_BOOL)(true);
            const existing = registeredDevices.get(devId);
            return (0, values_1.MK_BOOL)(existing ? existing.status === "revoked" : false);
        }
    });
        ...
        call: (args) => {
            ...
            httpServer = http.createServer((req, res) => {
                ...
                req.on("end", () => {
                    ...
                    let devicePubKey = req.headers["x-device-pubkey"] || "";
                    ...
                    if (hasEd25519Headers) {
                        ...
                        const verifyRes = device_1.DeviceIdentityManager.verifyPayloadSignature(pubKeyToUse, method, reqUrl, bodyData, nonce, timestamp, signature, 30000);
                        if (!verifyRes.valid) {
                            res.writeHead(401, { "Content-Type": "application/json" });
                            res.end(JSON.stringify({
                                status: 401,
                        ...
                        const verification = device_1.DeviceIdentityManager.verifyRemoteToken(deviceToken, authSecret);
                    ...
                    if (reqUrl === "/api/rpc" && method === "POST") {
                        try {
                            ...
                            const expectedParams = (fnToCall && fnToCall.type === "fn" && Array.isArray(fnToCall.parameters))
                                ? fnToCall.parameters.length
                                : -1;
                            const runtimeArgs = (expectedParams === rpcArgs.length)
                                ? rpcArgs.map(jsToRuntimeVal)
                                : [{ type: "instance", instance: reqInstance }, ...rpcArgs.map(jsToRuntimeVal)];
                            const execResult = (0, interpreter_1.callLLPFunction)(fnToCall, runtimeArgs, env);
                    ...
                    if (routeConfig && routeConfig.handler) {
                        try {
                            ...
                            const handlerResult = (0, interpreter_1.callLLPFunction)(fnToCall, [{ type: "instance", instance: reqInstance }], env);
                            let outputStr = "";
            ...
            if (rpcHandlers.has(localKey) || rpcHandlers.has(method)) {
                ...
                try {
                    ...
                    if (handler && handler.type === "string") {
                        try {
                            handler = env.lookupVar(handler.value);
                        }
                        catch (_) { }
                    }
                    const dummyReq = new instance_1.Instance("HttpRequest");
                    dummyReq.SetProperty("DeviceId", (0, values_1.MK_STRING)(deviceMgr.getDeviceId()));
                    dummyReq.SetProperty("ClientIp", (0, values_1.MK_STRING)("127.0.0.1"));
                    const expectedParams = (handler && handler.type === "fn" && Array.isArray(handler.parameters))
                        ? handler.parameters.length
                        : -1;
                    const runtimeArgs = (expectedParams === callArgs.length)
                        ? callArgs.map(jsToRuntimeVal)
                        : [{ type: "instance", instance: dummyReq }, ...callArgs.map(jsToRuntimeVal)];
                    const res = (0, interpreter_1.callLLPFunction)(handler, runtimeArgs, env);
                    return res;
                }
                catch (e) {
            ...
            if (devicePushQueues.has(devId) && (devicePushQueues.get(devId) || []).length > 0) {
                ...
                for (const ev of localEvents) {
                    ...
                    for (const l of listeners) {
                        try {
                            (0, interpreter_1.callLLPFunction)(l, [jsToRuntimeVal(ev.data)], env);
                        }
                        catch (_) { }
                    }
                }
