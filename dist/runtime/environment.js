        ...
        if (explicitType === "Global") {
            if (val.type === "number") {
                if (val.value < -32768 || val.value > 32767) {
                    throw new Error(`[LLP Type Error] La variable Global '${name}' dépasse le petit intervalle autorisé [-32768, 32767] (reçu ${val.value}).`);
                }
            }
            else if (val.type === "string") {
                if (val.value.length > 256) {
                    throw new Error(`[LLP Type Error] La variable Global '${name}' dépasse la taille maximale autorisée (max 256 caractères, reçu ${val.value.length}).`);
                }
            }
            else if (val.type === "list" || val.type === "fixed_array") {
                const len = val.elements ? val.elements.length : 0;
                if (len > 256) {
                    throw new Error(`[LLP Type Error] La variable Global '${name}' dépasse la taille maximale autorisée (max 256 éléments, reçu ${len}).`);
                }
            }
            return;
        }
        ...
        if (explicitType === "Json") {
            ...
            if (val.type === "string") {
                try {
                    ...
                    throw new Error(`[LLP Type Error] La variable '${name}' de type 'Json' attend une chaîne au format JSON valide ou un objet Json.`);
                }
            }
            if (val.type === "list") {
        ...
        if (explicitType === "Hexa") {
            ...
            if (val.type === "number") {
                if (!Number.isInteger(val.value)) {
                    throw new Error(`[LLP Type Error] La variable '${name}' de type 'Hexa' attend un entier hexadécimal (reçu float: ${val.value}).`);
                }
                return;
            }
            if (val.type === "string") {
                ...
                if (!isHex) {
                    throw new Error(`[LLP Type Error] La variable '${name}' de type 'Hexa' attend une chaîne hexadécimale valide (ex: 0xFF, '#FFA0', '1A2B', reçu '${val.value}').`);
                }
                return;
            }
            throw new Error(`[LLP Type Error] La variable '${name}' de type 'Hexa' attend un nombre ou une chaîne hexadécimale (reçu ${val.type}).`);
        }
