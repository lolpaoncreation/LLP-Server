...
exports.currentAppConfig = exports.currentServerInstance = void 0;
exports.stopGuiApplication = stopGuiApplication;
...
exports.currentServerInstance = null;
exports.currentAppConfig = {
    windowWidth: 1200,
    windowHeight: 860,
    locked: false,
    devMode: false,
...
function stopGuiApplication() {
    if (exports.currentServerInstance) {
        try {
            exports.currentServerInstance.close();
        }
        catch (_) { }
        exports.currentServerInstance = null;
    }
}
function setAppLock(locked = true) {
    exports.currentAppConfig.locked = locked;
}
function getAppConfig() {
    return exports.currentAppConfig;
}
function startGuiApplication(options = {}) {
    return new Promise((resolve, reject) => {
        ...
        const db = new cllpdb_1.CryptedLolpaonDatabase(effectiveDbPath);
        ...
        const server = http.createServer((req, res) => {
            ...
            if (pathname === "/api/products" && req.method === "POST") {
                ...
                readJsonBody().then(body => {
                    ...
                    let maxId = 100;
                    existingProds.forEach(p => { if (p.id > maxId)
                        maxId = p.id; });
                    const newId = maxId + 1;
            ...
            if (pathname.startsWith("/api/products/") && req.method === "DELETE") {
                ...
                if (!id)
                    return sendJson(400, { success: false, message: "ID invalide." });
            ...
            if (pathname === "/api/analytics/scillp" && req.method === "POST") {
                try {
                    const env = (0, index_1.createGlobalEnvironment)();
                    const llpCode = `
          ...
          `;
                    (0, index_1.executeLLP)(llpCode, env);
                    const meanVal = env.lookupVar("mean")?.value || 0;
                    const stdDevVal = env.lookupVar("stdDev")?.value || 0;
                    const smoothedVal = (env.lookupVar("smoothed")?.elements || []).map((e) => Math.round(e.value * 10) / 10);
                    return sendJson(200, {
                        success: true,
          ...
          `;
                    (0, index_1.executeLLP)(llpCode, env);
                    const derivVal = env.lookupVar("deriv")?.value || "";
                    const integVal = env.lookupVar("integ")?.value || "";
                    const solvedVal = (env.lookupVar("solved")?.elements || []).map((e) => e.value);
                    return sendJson(200, {
                        success: true,
                        derivative: derivVal,
                        integral: integVal,
          ...
          `;
                    (0, index_1.executeLLP)(llpCode, env);
                    const combVal = env.lookupVar("comb")?.value || 20;
                    const poisVal = env.lookupVar("pois")?.value || 0.1888;
                    return sendJson(200, {
                        success: true,
            ...
            if (pathname === "/api/app/config" && req.method === "GET") {
                return sendJson(200, exports.currentAppConfig);
            }
            if (pathname === "/api/app/lock" && req.method === "POST") {
                exports.currentAppConfig.locked = true;
            ...
            if (pathname === "/api/app/silence" && req.method === "POST") {
                exports.currentAppConfig.silence = true;
  ...
  <header class="app-header">
    <div class="brand">
      <div class="brand-icon" style="background: transparent; box-shadow: none;">
        <img src="${logo_1.LLP_LOGO_BASE64}" alt="LLP Logo" style="width: 28px; height: 28px; object-fit: contain;" />
      </div>
      <div>
    ...
    <div class="login-card">
      <div class="login-header">
        <div class="login-logo">
          <img src="${logo_1.LLP_LOGO_BASE64}" alt="LLP Logo" />
        </div>
