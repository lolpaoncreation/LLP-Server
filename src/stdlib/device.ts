...
export class DeviceIdentityManager {
  ...
  private hardwareSecret: string | null = null;
  private privateKey: crypto.KeyObject | null = null;
  private publicKeyPem: string = "";
  private isClonedOrTampered: boolean = false;

  private anchorFilePath: string;
  private keySealFilePath: string;
  private pubKeyFilePath: string;
   ...
   */
  private runCommand(cmd: string, timeoutMs: number = 2000): string {
    try {
      return execSync(cmd, {
        encoding: "utf8",
        timeout: timeoutMs,
   ...
   */
  private getMotherboardIdentity(): { uuid: string; serial: string } {
    let uuid = "";
    let serial = "";
    const p = os.platform();
    ...
    if (p === "win32") {
      ...
      if (uuidLines.length > 1 && uuidLines[1].trim() && uuidLines[1].trim() !== "00000000-0000-0000-0000-000000000000") {
        ...
        if (psUuid && psUuid !== "00000000-0000-0000-0000-000000000000") uuid = psUuid;
      }
      ...
      if (biosLines.length > 1 && biosLines[1].trim() && biosLines[1].trim().toLowerCase() !== "to be filled by o.e.m.") {
        ...
        if (psSerial && psSerial.toLowerCase() !== "to be filled by o.e.m.") serial = psSerial;
      }
    } else if (p === "linux") {
      // Linux DMI paths
      ...
      for (const sp of serialPaths) {
        if (fs.existsSync(sp)) {
          try {
            ...
            if (content) { serial = content; break; }
          } catch (_) {}
        }
      }
    } else if (p === "darwin") {
      ...
      if (uuidMatch) uuid = uuidMatch[1].trim();
   ...
   */
  private getSystemDiskIdentity(): string {
    const p = os.platform();
    let diskId = "";
      ...
      for (const cp of candidates) {
        if (fs.existsSync(cp)) {
          try {
            ...
            if (content) { diskId = content; break; }
          } catch (_) {}
        }
      }
    } else if (p === "darwin") {
   ...
   */
  public collectHardwareFingerprint(): { rawData: string; fingerprint: string } {
  ...
  private initializeIdentity(): void {
    ...
    if (!fs.existsSync(anchorDir)) {
      try { fs.mkdirSync(anchorDir, { recursive: true }); } catch (_) {}
    }
   ...
   */
  private initializeSealedKeypair(deviceFingerprint: string): void {
   ...
   * StringToSign = Nonce + "\n" + Timestamp + "\n" + HTTP_Method + "\n" + CanonicalPayload
   */
  public signPayload(method: string, path: string, payload: string): {
    deviceId: string;
    nonce: string;
    timestamp: string;
    signature: string;
    ...
    if (!this.privateKey) {
      throw new Error(
        "[LLP Device Security] Clé privée Ed25519 indisponible : l'empreinte matérielle de cette machine ne correspond pas au scellement local (Tentative de clonage ou fichier corrompu)."
      );
    }
   ...
   */
  public static verifyPayloadSignature(
    publicKeyPem: string,
    method: string,
    path: string,
    payload: string,
   ...
   */
  public generateDeviceToken(payload: string = ""): string {
    const deviceId = this.getDeviceId();
    const timestamp = Date.now().toString();
   ...
   */
  public verifyDeviceToken(token: string, payload: string = "", maxAgeMs: number = 300000): boolean {
   ...
   */
  public static verifyRemoteToken(token: string, secret?: string, payload: string = "", maxAgeMs: number = 300000): { valid: boolean; error?: string } {
    if (!token || typeof token !== "string") return { valid: false, error: "Jeton manquant" };
    const parts = token.split(".");
    if (parts.length !== 4) return { valid: false, error: "Format de jeton invalide" };

    const [deviceId, timestampStr, nonce, signature] = parts;
    const timestamp = parseInt(timestampStr, 10);
    if (isNaN(timestamp)) return { valid: false, error: "Horodatage invalide" };

    if (Math.abs(Date.now() - timestamp) > maxAgeMs) return { valid: false, error: "Jeton expiré" };
 ...
 */
export function registerDevice(env: Environment) {
  const deviceMgr = DeviceIdentityManager.getInstance();
  const deviceObj = new Instance("DeviceSecurityService");
  ...
  deviceObj.SetProperty("GetToken", {
    ...
    call: (args: RuntimeVal[]) => {
      ...
      return MK_STRING(deviceMgr.generateDeviceToken(payload));
    }
  });
  ...
  deviceObj.SetProperty("Sign", {
    ...
    call: (args: RuntimeVal[]) => {
      ...
      return MK_STRING(deviceMgr.signData(data));
    }
  });
  ...
  deviceObj.SetProperty("SignPayload", {
    ...
    call: (args: RuntimeVal[]) => {
      ...
      const sigInst = new Instance("PayloadSignature");
      sigInst.SetProperty("nonce", MK_STRING(sigData.nonce));
      sigInst.SetProperty("timestamp", MK_STRING(sigData.timestamp));
      sigInst.SetProperty("signature", MK_STRING(sigData.signature));
      sigInst.SetProperty("deviceId", MK_STRING(sigData.deviceId));
      sigInst.SetProperty("publicKey", MK_STRING(sigData.publicKeyPem));
      sigInst.SetProperty("json", MK_STRING(JSON.stringify(sigData)));
  ...
  deviceObj.SetProperty("Verify", {
    ...
    call: (args: RuntimeVal[]) => {
      ...
      return MK_BOOL(deviceMgr.verifyDeviceToken(token, payload));
    }
  });
  ...
  deviceObj.SetProperty("GetPlatform", {
    type: "native_fn",
    call: () => MK_STRING(deviceMgr.getPlatform())
  });

  // Device.GetInfo() -> String (JSON)
  deviceObj.SetProperty("GetInfo", {
    type: "native_fn",
    call: () => MK_STRING(JSON.stringify(deviceMgr.getInfo()))
  });

  // Device.IsTrusted() -> Boolean
  deviceObj.SetProperty("IsTrusted", {
    type: "native_fn",
    call: () => MK_BOOL(deviceMgr.isTrusted())
  });
