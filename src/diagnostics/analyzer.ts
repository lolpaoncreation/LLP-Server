...
export function analyzeSource(source: string, filePath: string = "source.llp"): DiagnosticItem[] {
  ...
  const bracketStack: DelimiterItem[] = [];
  const braceStack: DelimiterItem[] = [];
  const blockStack: DelimiterItem[] = []; // for if, while, for, function, class, module, namespace ... then ... end
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let col = 0; col < rawLine.length; col++) {
      ...
      if ((ch === '"' || ch === "'") && prev !== "\\") {
        if (!inStrChar) { inStrChar = true; quoteChar = ch; }
        else if (ch === quoteChar) { inStrChar = false; }
      }

      if (inStrChar) continue;
